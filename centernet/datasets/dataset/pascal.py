...
class PascalVOC(data.Dataset):
    ...
    def convert_eval_format(self, all_bboxes):
        detections = [[[] for __ in range(self.num_samples)] for _ in range(self.num_classes + 1)]
...
# noinspection PyAbstractClass
class AlgeaVOC(data.Dataset):  # it's actually coco though
    num_classes = 20
    default_resolution = [384, 384]
    mean = np.array([0.485, 0.456, 0.406],
                    dtype=np.float32).reshape(1, 1, 3)
    ...
    def __init__(self, opt, split):
        super(AlgeaVOC, self).__init__()
        opt.data_dir = '/media/palm/data/MicroAlgae'
        self.data_dir = os.path.join(opt.data_dir)
        self.img_dir = os.path.join(self.data_dir, 'images')
        _ann_name = {'train': 'train', 'val': 'test2007'}
        self.annot_path = os.path.join(self.data_dir, 'annotations', '{}_algea.json').format(split)
        self.max_objs = 50
        self.class_name = ['ov', 'mif']
    ...
    def convert_eval_format(self, all_bboxes):
        ...
        for image_id in all_bboxes:
            for cls_ind in all_bboxes[image_id]:
                print(cls_ind)
                category_id = self._valid_ids[cls_ind]
    ...
    def save_results(self, results, save_dir):
        print(save_dir.split('/'))
        import pickle as pk
        pk.dump(results, open('{}/raw_results.json'.format(save_dir), 'wb'))
        json.dump(self.convert_eval_format(results),
                  open('{}/results.json'.format(save_dir), 'w'))
